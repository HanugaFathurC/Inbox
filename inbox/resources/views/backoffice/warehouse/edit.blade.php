@extends('layouts.backoffice.master', ['title' => 'Edit Gudang'])

@section('content')
    <x-container>
        <div class="col-12">
            <x-card title="Edit Gudang" class="card-body">
                <form action="{{ route('backoffice.warehouse.update', $warehouse->id) }}" method="POST"
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <x-input title="Nomor Telpon" name="telp" type="tel"
                                placeholder="Masukan Nomor Telpon Gudang" value="{{ $warehouse->telp }}" />
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-6 ">
                        ...
                        <div class="col-6">
                            <x-select title="Tipe" name="type_id" id="type_id">
                                ...
                                @foreach ($types as $type)
                                    <option value="{{ $type->id }}" @selected($warehouse->type_id == $type->id)>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </x-select>
                        ...
                        <div class="col-6">
                            <x-select title="Provinsi" name="indonesia_provinces_id" id="indonesia_provinces_id">
                                ...
                                @foreach ($provinces as $item)
                                    <option value="{{ $item->id }}" @selected($warehouse->indonesia_provinces_id == $item->id)>
                                        {{ $item->name }}</option>
                                @endforeach
                            </x-select>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-6">
                            <x-select title="Kecamatan" name="indonesia_districts_id" id="indonesia_districts_id">
                                ...
                                <option value="{{ $distric->id }}" selected>
                                    {{ $distric->name }}</option>
                            </x-select>
                        </div>
                        <div class="col-6">
